...
under the OWNER'S account identity — the orchestrator has no separate GitHub
identity, so every post it makes is authored by the owner's account. That is
exactly the surface where the WO-93-98 fabricated-authorization incident lived,
so the bridge's own traffic is audited: automation that polices frozen surfaces
must itself be policed.
...
1. DISCLOSURE LINES: every issue/comment that dispatches or instructs Codex
...
2. FROZEN-MERGE BOUNDARY: no PR whose diff touches a frozen or registered
   ...
   conditions, ledger_anchor.py, docs/EXPERIMENT_REGISTRY.md, custody docs) was
   merged by automated/orchestrator activity. An owner-initiated merge is fine;
   verify frozen PRs describe themselves as owner-merge and were not self-merged
   by a routine run.
