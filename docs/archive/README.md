...
designed and what was tried, not because any of it is in force. The current
documents are listed in the repository `README.md`; where one of them replaces a
file here, the row names it. A replacement is the file that now covers the same
ground, which is not always a canonical one.

`reason` is one of three:

- **dated snapshot** — a note written about a particular day, correct as of that
  day and never updated since.
- **superseded by `<path>`** — a current file now covers the same ground; the
...
| `POLYMARKET_MISPRICING_BOT.md` | superseded by `docs/POLYMARKET_VPS_DOCKER_RUNBOOK.md` | `docs/POLYMARKET_VPS_DOCKER_RUNBOOK.md` |
