...
def _diagnose_no_trade(*, readiness: dict[str, Any], broker: dict[str, Any], predictions: list[dict[str, Any]], approved: list[dict[str, Any]], rejected: list[dict[str, Any]]) -> dict[str, Any]:
    if approved:
        return {"status": "signals_available", "main_blocker": "Approved paper signals exist; broker should process them unless duplicate, risk, or pause controls intervene.", "next_action": "Monitor broker fills, duplicate skips, and paper P&L."}
    entry_pause = str(broker.get("entry_pause_reason") or "").strip()
    if entry_pause:
        return {"status": "entry_paused", "main_blocker": entry_pause, "next_action": "Monitor exits and wait for equity to recover above the clean forward P&L pause threshold."}
    if not readiness.get("approved_for_paper_trading"):
        blockers = readiness.get("blockers") or []
        return {"status": "paper_readiness_blocked", "main_blocker": "; ".join(str(item) for item in blockers) or "paper readiness is not approved", "next_action": "Fix the readiness blocker before expecting paper entries."}
    if not predictions:
        return {"status": "no_predictions", "main_blocker": "No prediction rows are available for the current cycle.", "next_action": "Run or wait for discovery plus prediction so the signal gate has a scored universe."}
    if rejected:
    ...
    return {"status": "no_signal_evidence", "main_blocker": "No approved signals and no rejected-signal evidence are available.", "next_action": "Wait for the next prediction cycle or inspect the local-live heartbeat for stalled discovery/prediction."}


def build_cycle_decision_trace(cfg: EngineConfig, *, cycle_report: dict[str, Any] | None = None, broker_summary: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    no_trade = _diagnose_no_trade(readiness=readiness, broker=broker, predictions=predictions, approved=approved, rejected=rejected)
...
def _subsystem_freshness(cfg: EngineConfig) -> dict[str, Any]:
    ...
    for name, path in candidates.items():
        ...
        rows.append({"subsystem": name, "path": str(path), "status": _status_from_payload(payload), "generated_at_utc": _payload_timestamp(payload), "age_seconds": _payload_age_seconds(payload)})
...
def write_agent_runtime_bundle(cfg: EngineConfig, *, cycle_report: dict[str, Any] | None = None, broker_summary: dict[str, Any] | None = None) -> dict[str, Any]:
