"""WO-99 leakage-safe resolved-corpus training assembly.
...
def training_settings(cfg: EngineConfig) -> dict[str, float]:
    ...
    *,
    ...
    for row in iter_csv_rows(path):
        ...
        if close_at is None or resolution_at is None:
            ...
            continue
        label_available = max(close_at.astimezone(timezone.utc), resolution_at.astimezone(timezone.utc))
    ...
    *,
    ...
    if len(ordered) < 2:
        ...
        validation_count = min(
            len(ordered) - 1,
            max(1, int(math.ceil(len(ordered) * settings["validation_fraction"]))),
        )
    ...
    for market_id in ordered:
        ...
        if len(ordered) < 2:
            reason = "need_at_least_two_independent_markets"
...
def build_leakage_safe_training(
    ...
    *,
        ...
        features.append(
            {
                ...
                "split": split,
                "feature_source": "wo99_historical_bid_ask_v1",
                "midpoint": midpoint,
    ...
    status = (
        ...
        and split_summary["validation_markets"] > 0
    ...
    payload = {
        ...
        "split_policy": "whole-market chronological validation tail; purge training labels overlapping validation feature start plus embargo",
        "settings": settings,
        "input_artifacts": {
        ...
        "next_unmet_condition": (
            ...
            if status == "ready"
            else "collect more exact bid/ask rows that later join to non-overlapping clean resolutions with both target classes"
        ),
    }
