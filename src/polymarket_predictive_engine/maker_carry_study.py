    ...
    for side, levels in (("bid", bids), ("ask", asks)):
        for level in levels:
            ...
            if price is None or size is None:
                continue
            scores[side] += _quadratic_score(abs(mid - price), v, size)
    return {"mid": mid, "bid_score": scores["bid"], "ask_score": scores["ask"]}
...
def _pickoff_window(
    ...
    *,
    ...
    try:
        ...
        return None
    prices = [safe_float(point.get("p")) for point in history if isinstance(point, dict)]
...
def _adverse_selection(
    settings: dict[str, Any], token_id: str, quote_distance: float, quote_size: float
) -> dict[str, Any] | None:
    """Charge the WORSE of the two windows.
    ...
    the 1-minute window misses, and vice versa.
    """
    fast = _pickoff_window(
        settings,
        token_id,
        quote_distance,
        quote_size,
        interval="1d",
        fidelity_minutes=max(1, int(settings["reaction_minutes"])),
        min_points=30,
    )
    slow = _pickoff_window(
        settings,
        token_id,
        quote_distance,
        quote_size,
    ...
    if fast is None and slow is None:
        return None
    charge = max(row["adverse_usd_per_day"] for row in (fast, slow) if row is not None)
    ...
    return {
        ...
        "adverse_selection_usd_per_day": charge,
...
def run_maker_carry_study(cfg: EngineConfig) -> dict[str, Any]:
    ...
    for market in universe[: int(settings["max_book_candidates"])]:
        ...
        adverse = _adverse_selection(settings, market["token_id"], quote_distance, quote_size)
        row = {
            **market,
    ...
    for row in sorted(trusted, key=lambda r: r["net_carry_usd_per_day"], reverse=True):
        ...
        for k in range(1, max_multiple + 1):
            ...
            net_k = row["pot_usd_per_day"] * share_k - k * row["adverse_selection_usd_per_day"]
    ...
    summary.update(
        {
            ...
            "assumptions": {
                ...
                "adverse_selection_model": (
                    f"full-size fill on every bar move beyond quote distance; charge = worse of "
                    f"24h@{settings['reaction_minutes']}min and 7d@10min windows"
                ),
