...
Design choices that keep the test honest:
  ...
  - WO-99's precomputed split is chronological **by market**, purges every
...
from ..leakage_safe_training import FEATURES_RELATIVE_PATH, LABELS_RELATIVE_PATH
...
MODEL_VERSION = "pm-skill-logit-v2-wo99"
...
def _joined_registered_rows(feature_path: str, label_path: str) -> list[dict[str, Any]]:
    """Join the separated WO-99 files by immutable quote-derived row id."""

    labels = {
        str(row.get("training_row_id") or ""): row
        for row in read_csv_rows(label_path)
        if str(row.get("training_row_id") or "")
    }
    ...
    for feature in read_csv_rows(feature_path):
        ...
        if label is None:
            continue
        if str(feature.get("split") or "") != str(label.get("split") or ""):
            continue
...
def _registered_split_indices(rows: list[dict[str, Any]]) -> tuple[list[int], list[int], set[str]]:
    train_idx = [index for index, row in enumerate(rows) if row.get("split") == "train"]
    validation_idx = [index for index, row in enumerate(rows) if row.get("split") == "validation"]
    train_markets = {
...
def _executable_edge_roi(
    ...
    """Long token-level ROI at recorded ask plus the canonical taker fee.

    Binary complements are separate token rows.  The evaluator therefore buys
    only the observed token when model fair clears its actual ask; it never
    manufactures a complement quote from ``1 - midpoint``.
    """
...
def train_skill_model(cfg: EngineConfig, test_fraction: float = 0.3, l2: float = 5.0,
    ...
    rows = _joined_registered_rows(
        str(cfg.output_root / FEATURES_RELATIVE_PATH),
        str(cfg.output_root / LABELS_RELATIVE_PATH),
    )
    ...
    if len(design) < 50 or not test_idx or len({design[i]["_y"] for i in train_idx}) < 2 or market_overlap:
        summary = {
            ...
            "markets": n_markets,
            "reason": (
                "need >=50 WO-99 exact bid/ask rows, both classes in the purged train split, "
                "a non-empty registered validation split, and zero market overlap"
            ),
            "market_overlap": sorted(market_overlap),
            "input_features": str(cfg.output_root / FEATURES_RELATIVE_PATH),
            "input_labels": str(cfg.output_root / LABELS_RELATIVE_PATH),
    ...
    summary = {
        ...
        "model_version": MODEL_VERSION,
        "generated_at_utc": now_utc(),
        "input_contract": "WO-99 append-only resolutions + exact historical bid/ask + purged chronological split",
        "evidence_class": "diagnostic_h3_structural_bias_substrate",
        ...
        "split": {
            "policy": "precomputed WO-99 whole-market purged chronological split",
            "train_rows": len(train_idx),
            "test_rows": len(test_idx),
            "train_markets": len({str(design[i].get("market_id") or design[i].get("market_slug")) for i in train_idx}),
            "test_markets": len(set(markets)),
        ...
        "oos_edge_roi": {
            "note": "Diagnostic only: buy the observed token at its recorded best ask, charge the canonical category/price-aware taker fee, and settle at resolution. No midpoint is executable.",
            "by_threshold": [
