...
def _market_family(row: dict[str, Any]) -> str:
    ...
    if "updown" in slug or "-up-or-down" in slug or " up or down " in f" {text} ":
        if slug.startswith("btc") or "bitcoin" in text:
            asset = "btc"
        elif slug.startswith("eth") or "ethereum" in text:
            asset = "eth"
        elif slug.startswith("sol") or "solana" in text:
            asset = "sol"
        elif slug.startswith("xrp") or "ripple" in text:
            asset = "xrp"
        else:
            asset = ""
        if "updown-5m" in slug or re.search(r"\b5\s*min", text):
            interval = "5m"
        elif "updown-15m" in slug or re.search(r"\b15\s*min", text):
            interval = "15m"
        else:
            interval = "event"
