...
# A timeout is coverage-only only for the pure price-history collector. The
# maker-carry study produces registered gate evidence, so its timeout remains a
# loud harvest failure rather than allowing stale study artifacts downstream.
COLLECTION_TIMEOUT_TOLERATED_STEPS = frozenset({"collect_price_history"})
...
def _steps(config_path: str) -> list[HarvestStep]:
    regular = [
        ...
        HarvestStep("h2_dutch_evaluate", _engine_command("h2-dutch-evaluate", config_path)),
...
def _latency_summary(path: Path) -> list[dict[str, Any]]:
    ...
    return [
        {
            ...
            "p50_seconds": round(float(statistics.median(values)), 3),
            "max_seconds": round(max(values), 3),
        }
        for (host, path_tail), values in sorted(grouped.items())
    ]
...
def run_training_harvest(
    ...
    *,
    ...
    for step in _steps(config_path):
        ...
        previous_latency_log = os.environ.get("PM_HARVEST_LATENCY_LOG")
        os.environ["PM_HARVEST_LATENCY_LOG"] = str(latency_path)
        try:
            ...
            if previous_latency_log is None:
