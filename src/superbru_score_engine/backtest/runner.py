...
def run_backtest(args: argparse.Namespace, config: AppConfig) -> int:
    ...
    for rho in rho_grid:
        for ci_cutoff in ci_grid:
            ...
            avg_model = float(frame["model_points"].mean())
            avg_naive = float(frame["naive_points"].mean())
    ...
    summary = {
        ...
        "avg_model_points": float(best_frame["model_points"].mean()),
        "avg_naive_points": float(best_frame["naive_points"].mean()),
        "edge_vs_naive": float(best_frame["model_points"].mean() - best_frame["naive_points"].mean()),
...
def naive_baseline_pick(match, distribution: DistributionResult) -> tuple[int, int]:
    ...
    favourite = max(range(3), key=lambda idx: values[idx])
