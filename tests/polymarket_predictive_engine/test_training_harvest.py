...
def test_maker_carry_timeout_fails_loud_and_later_children_still_run(tmp_path: Path) -> None:
    cfg, config_path = _config(tmp_path)
    calls: list[str] = []
    ...
    def runner(command: Sequence[str], timeout_seconds: int) -> int:
        ...
        name = _step_name(command)
        calls.append(name)
        return 124 if name == "maker-carry-study" else 0

    payload = run_training_harvest(cfg, config_path=str(config_path), runner=runner, clock=lambda: 0.0)
    row = next(row for row in payload["steps"] if row["step"] == "maker_carry_study")
    assert row["status"] == "timed_out"
    assert row["timeout_degrades_coverage"] is False
    assert payload["status"] == "partial_failure"
    assert payload["failed_steps"] == ["maker_carry_study"]
    assert calls.index("decision-policy") > calls.index("maker-carry-study")
...
def test_degraded_harvest_retries_after_retry_interval_same_day(tmp_path: Path) -> None:
    ...
    (out_dir / "last_completion_training_harvest").write_text(str(now - 1_801), encoding="utf-8")
    subprocess.run(
...
def test_degraded_retry_backoff_starts_at_completion(tmp_path: Path) -> None:
    out_dir = tmp_path / "ops"
    out_dir.mkdir()
    now = int(subprocess.check_output(["date", "-u", "+%s"], text=True).strip())
    (out_dir / "last_success_training_harvest").write_text(str(now - 90_000), encoding="utf-8")
    # The attempt may have started hours ago, but it has only just completed.
    (out_dir / "last_completion_training_harvest").write_text(str(now), encoding="utf-8")
...
def test_scheduler_rearms_harvest_from_successful_completion_not_start() -> None:
    ...
    assert "touch_completion_stamp training_harvest" not in harvest_block
    assert "touch_completion_stamp training_harvest" in function
