...
def test_timed_out_child_fails_harvest_but_later_children_still_run(tmp_path: Path) -> None:
    cfg, config_path = _config(tmp_path)
    calls: list[str] = []
    ...
    def runner(command: Sequence[str], timeout_seconds: int) -> int:
        ...
        name = _step_name(command)
        calls.append(name)
        return 124 if name == "collect-price-history" else 0

    payload = run_training_harvest(
        cfg, config_path=str(config_path), runner=runner, clock=lambda: 0.0
    )

    timed_out = next(row for row in payload["steps"] if row["step"] == "collect_price_history")
...
def test_latency_summary_is_functional_and_secret_safe(tmp_path: Path) -> None:
    cfg, config_path = _config(tmp_path)
...
def test_scheduler_rearms_harvest_from_successful_completion_not_start() -> None:
    ...
    assert "touch_completion_stamp training_harvest" in harvest_block
    assert harvest_block.index("run_training_harvest") < harvest_block.index(
...
def test_retry_backoff_is_measured_from_previous_completion(tmp_path: Path) -> None:
    out_dir = tmp_path / "ops"
    out_dir.mkdir()
    completion = out_dir / "last_completion_training_harvest"
    completion.write_text("9999999999\n", encoding="utf-8")
    env = {
        **os.environ,
    ...
    result = subprocess.run(
        ["sh", "-c", '. "$1"; training_harvest_retry_ready', "sh", "scripts/run_vps_ops_scheduler.sh"],
        env=env,
        check=False,
    )
